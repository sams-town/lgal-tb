...
$no_edaran     = $doc['nomor_surat']    ?? $_GET['no_edaran']    ?? '';
$tentang       = $doc['perihal']        ?? $_GET['tentang']      ?? '';
...
function fmtTglEdaran($tgl): string {
    if (!$tgl) return 'xx xxxxx xxxxx (tanggal, bulan, tahun)';
...
$tgl_tampil = fmtTglEdaran($tanggal);
...
<title>Surat Edaran - <?= htmlspecialchars($no_edaran ?: 'Preview') ?></title>
<style>
...
.edaran-title .no-edaran {
    font-size: 10.5pt;
    color: #1a2e3b;
    margin-bottom: 4px;
}
.edaran-title .tentang-label {
    font-size: 10.5pt;
    margin-bottom: 2px;
}
.edaran-title .tentang-val {
    font-size: 11pt;
    font-weight: bold;
    color: #1a5f6e;
    letter-spacing: .3px;
...
.ttd-block .ttd-kota  { margin-bottom: 2px; }
.ttd-block .ttd-inst  { font-weight: bold; margin-bottom: 44px; }
.ttd-block .ttd-nama  { font-weight: bold; text-decoration: underline; }
.ttd-block .ttd-jab   { font-size: 10pt; }
...
<div class="page">
  ...
  <!-- JUDUL -->
  <div class="edaran-title">
    <h1>Surat Edaran</h1>
    <div class="no-edaran">No.: <?= htmlspecialchars($no_edaran ?: 'XXX/SE-DIR/RSTHB/BULAN/TAHUN') ?></div>
    <div class="tentang-label">Tentang</div>
    <div class="tentang-val"><?= htmlspecialchars($tentang ?: 'XXXXXXXXXXXXXXXXX') ?></div>
  </div>

  <!-- ISI -->
  <div class="isi-edaran"><?= nl2br(htmlspecialchars($isi ?: "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx\nxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx.\n\nxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx\nxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx.")) ?></div>

  <!-- PENUTUP -->
  <div class="penutup">Demikian surat edaran ini dibuat dan disampaikan untuk dapat ditindaklanjuti. Atas perhatian dan kerja samanya disampaikan terima kasih.</div>
  ...
  <div class="ttd-wrap">
    <div class="ttd-block">
      ...
      <div class="ttd-nama"><?= htmlspecialchars($nama_ttd ?: 'Nama') ?></div>
      <div class="ttd-jab"><?= htmlspecialchars($jabatan_ttd ?: 'Jabatan') ?></div>
    </div>
  </div>
