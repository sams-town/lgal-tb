...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['tambah_sip_dokter'])) {
    if (!canUserEditOrDelete('komite')) {
        ...
        try {
            ...
            $_SESSION['success_msg'] = "Data SIP Dokter berhasil ditambahkan";
        } catch (PDOException $e) {
            $_SESSION['error_msg'] = "Gagal menambahkan data: " . $e->getMessage();
        }
    }
    header("Location: sip-dokter.php");
    exit;
}

// Handle form submission for editing SIP Dokter
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_sip_dokter'])) {
    if (!canUserEditOrDelete('komite')) {
        ...
        $stmt = $pdo->prepare("SELECT file_str, file_sip, file_pks, file_sk FROM tenaga_medis WHERE id = ? AND tipe_form = 'sip-dokter'");
        $stmt->execute([$editId]);
        ...
        try {
            ...
            $_SESSION['success_msg'] = "Data SIP Dokter berhasil diperbarui";
        } catch (PDOException $e) {
...
// Get data
try {
    $stmt = $pdo->prepare("SELECT * FROM tenaga_medis WHERE tipe_form = 'sip-dokter' ORDER BY created_at DESC");
    $stmt->execute();
...
<head>
    ...
    <title>SIP Dokter - RS Taman Harapan Baru</title>
...
<body class="min-h-screen bg-gray-50 flex">
        ...
        <div class="flex-1 p-8 overflow-y-auto">
            <div class="space-y-6">
                <div class="flex items-center justify-between">
                    <div>
                        <h1 class="text-3xl font-bold text-gray-900">SIP Dokter</h1>
                        <p class="text-gray-600 mt-2">Input dan manajemen Surat Izin Praktik Dokter</p>
                    </div>
                ...
                <div class="flex items-center gap-3 mb-4">
                    <a href="export_handler.php?action=export_data&module=tenaga_medis&subpage=sip-dokter" class="flex items-center gap-2 bg-emerald-600 text-white px-4 py-2 rounded-xl font-medium hover:bg-emerald-700 transition-colors">
                        📊
                                            ...
                                            <td class="px-6 py-4">
                                                <div class="flex items-center gap-2">
                                                    <?php if (canUserEditOrDelete('komite')): ?>
                                                        ...
                                                        <a href="sip-dokter.php?delete=<?php echo $data['id']; ?>" onclick="return confirm('Apakah Anda yakin ingin menghapus data ini?');" class="px-3 py-1 text-sm bg-red-100 text-red-700 rounded-lg hover:bg-red-200 transition-colors">Hapus</a>
                                                    <?php else: ?>
                                                        <span class="text-gray-400 text-sm">-</span>
                                                    <?php endif; ?>
                                                </div>
    ...
    <div id="modal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50">
        <div class="bg-white rounded-2xl shadow-xl max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            <div class="p-6 border-b border-gray-100 flex items-center justify-between">
                <h2 id="modal-title" class="text-xl font-bold text-gray-900">Form Input SIP Dokter</h2>
            ...
            <form method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
                ...
                <div class="flex gap-3 pt-4">
                    ...
                    <button type="submit" name="tambah_sip_dokter" id="submit-btn" class="flex-1 px-4 py-2 bg-emerald-600 text-white rounded-xl font-medium hover:bg-emerald-700 transition-colors">
                        Simpan Data
                    </button>
                </div>
            </form>
    ...
    <div id="importModal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50">
        <div class="bg-white rounded-2xl shadow-xl max-w-md w-full mx-4">
            ...
            <form method="POST" action="export_handler.php?action=import_data&module=tenaga_medis&subpage=sip-dokter" enctype="multipart/form-data" class="p-6 space-y-4">
                <div>
    ...
    <script>
        ...
        window.openModal = function(modalId) {
            if (modalId === 'modal' || !modalId) {
                resetForm();
                document.getElementById('submit-btn').name = 'tambah_sip_dokter';
                document.getElementById('submit-btn').textContent = 'Simpan Data';
                document.getElementById('modal-title').textContent = 'Form Input SIP Dokter';
            }
        ...
        function openEditModal(doc) {
            ...
            document.getElementById('submit-btn').name = 'edit_sip_dokter';
            document.getElementById('submit-btn').textContent = 'Simpan Perubahan';
            document.getElementById('modal-title').textContent = 'Edit SIP Dokter';
